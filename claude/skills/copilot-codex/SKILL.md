...
2. **Gather context**: read relevant files so copilot can work without exploring itself. Include actual code, not placeholders.
3. **Write prompt file**: save a detailed prompt to `/tmp/copilot-prompt.txt`.
...
Use `scripts/run_copilot.sh` to handle CLI flags, escaping, and session ID extraction:
...
The prompt is the single most important input. Include:

- **Actual code** from relevant files (not summaries or placeholders)
- **Goal and requirements** with specific acceptance criteria
- **Constraints**: language, framework, coding standards, backward compatibility
- **Existing patterns**: tests, validation logic, naming conventions

For task-specific prompt templates, see [references/examples.md](references/examples.md).
