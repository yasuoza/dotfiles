...
1. **Consensus detection**: Match findings by file path + line range + issue category. Findings within 5 lines of each other on the same topic count as consensus.
2. **Severity ranking**: Critical > High > Medium > Low. Consensus findings are automatically elevated by one severity level.
3. **Deduplication**: For consensus items, prefer the more detailed explanation. Attribute to both reviewers.
