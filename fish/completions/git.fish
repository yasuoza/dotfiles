...
if type hub >/dev/null
  ### create
  complete -c git -n '__fish_git_needs_command' -a create -d 'create a new Github repository'
  complete -f -c git -n '__fish_git_using_command create' -s p -d 'Private repository'
  complete -f -c git -n '__fish_git_using_command create' -s d -d 'Repository description'
  complete -f -c git -n '__fish_git_using_command create' -s h -d 'Repository homepage'

  ### browse
  complete -c git -n '__fish_git_needs_command' -a browse -d "Open repository's GitHub page"

  ### fork
  complete -c git -n '__fish_git_needs_command' -a fork -d 'Forks the original project'

  ### pull-request
  complete -c git -n '__fish_git_needs_command' -a pull-request -d 'Open new pull-request'
  complete -f -c git -n '__fish_git_using_command pull-request' -s b -a '(__fish_git_branches)' -d 'BASE'
  complete -f -c git -n '__fish_git_using_command pull-request' -s h -a '(__fish_git_branches)' -d 'HEAD'

  ### ci-status
  complete -c git -n '__fish_git_needs_command' -a ci-status -d 'Looks up the SHA for COMMIT in GitHub Status API'
  complete -f -c git -n '__fish_git_using_command ci-status' -s h -a '(__fish_git_branches)' -d 'Branch'
  complete -f -c git -n '__fish_git_using_command ci-status' -s h -a '(__fish_git_tags)' -d 'Tag'
